% Full Waveform Inversion - Code No.D3.
...
PathSrc = '/Users/bcbirkel/Documents/GitHub/LABasin/Hercules/SeptLaHabraRun/Simulation/Forward/';
PathSrc0 = '/Users/bcbirkel/Documents/GitHub/LABasin/Hercules/oldSim/S20140329/Simulation/Forward/';

%% UserModification: Path to the Resources directory (catalogs, stations...)
PathRes='/Users/bcbirkel/Documents/GitHub/LABasin/Hercules/SeptLaHabraRun/Resources/';
...
PathAux = '/Users/bcbirkel/Documents/GitHub/LABasin/Hercules/SeptLaHabraRun/Codes/AuxFiles/';
...
FMax=1;
FMin=1/100;
Mo= 4.065432e+30; % Seismic Moment from Hercules MUST CHANGE BASED ON SIM!!
Dt=0.25;
T=0:Dt:200;
T_iris = 0:0.05:200;
...
% Ploting recording at each station. Open plane
PathPlane0=[PathSrc 'Source1/output/planes/planedisplacements.0'];
Plane0=fopen(PathPlane0);

% Header and auxiliary data
Aux0=  fread(Plane0, 8,'double');
BoxCorners0=reshape(Aux0,4,2);
LengthY0=  fread(Plane0, 1,'double');
LengthX0=  fread(Plane0, 1,'double');
PointsDip0 = fread(Plane0, 1,'int');
PointsStrike0    = fread(Plane0, 1,'int');
DeltaT0  = fread(Plane0, 1,'double');
Steps0      = fread(Plane0, 1,'int');
TimeSteps0 = round(T(end)/(Steps0*DeltaT0)+1);

% Reading Components for all stations
for nM=1:TimeSteps0
    for iE=1:PointsStrike0
        SynS0(iE).X(nM) = fread(Plane0, 1,'double');
        SynS0(iE).Y(nM) = fread(Plane0, 1,'double');
        SynS0(iE).Z(nM) = fread(Plane0, 1,'double');
    end
end
fclose(Plane0);
...
%   ORIGINAL SIMULATION
    SX0 = Factor*(SynS0(nE).X);
    SY0 = Factor*(SynS0(nE).Y);
    SZ0 = -Factor*(SynS0(nE).Z);
    ...
    if norm(DN)==0 || norm(DE)==0
        ...
        SY0 = cosd(An)*DESR+sind(An)*DNSR;
        SX0 = -sind(An)*DESR+cosd(An)*DNSR;
        clear DXS DYS DNSR DESR;
    end
    ...
    Syn0(1).Signal = cosd(AnR)*SY0+sind(AnR)*SX0;
    Syn0(2).Signal = -sind(AnR)*SY0+cosd(AnR)*SX0;
    Syn0(3).Signal = SZ0;
    
    clear DX DY DZ SX0 SY0 SZ0;
    
    t25 = tukeywin(length(Syn0(1).Signal),0.05);
    CMP = ['-R';'-T';'-Z'];
    [Am,In]=max(max(abs([Obs(1).Signal; Obs(2).Signal; Obs(3).Signal])));
    if Am==0
        Am=1;
    ...
    cd '/Users/bcbirkel/Documents/GitHub/LABasin/Hercules/SeptLaHabraRun/SACfiles'
    eventtime = datenum(Eqk(iS).Loc.Year,Eqk(iS).Loc.Month,Eqk(iS).Loc.Day,Eqk(iS).Loc.Hour,Eqk(iS).Loc.Min,Eqk(iS).Loc.Sec);
    mksac([Eqk(iS).Stn(nE).Name '_R.SAC'],Obs(1).Signal,eventtime,'DELTA',Dt,'KSTNM',[Eqk(iS).Stn(nE).Name],'KCMPNM','R');
    mksac([Eqk(iS).Stn(nE).Name '_T.SAC'],Obs(2).Signal,eventtime,'DELTA',Dt,'KSTNM',[Eqk(iS).Stn(nE).Name],'KCMPNM','T');
    mksac([Eqk(iS).Stn(nE).Name '_Z.SAC'],Obs(3).Signal,eventtime,'DELTA',Dt,'KSTNM',[Eqk(iS).Stn(nE).Name],'KCMPNM','Z');
    
%% PLOTTING 
    cd '/Users/bcbirkel/Documents/GitHub/LABasin/Hercules/SeptLaHabraRun/Codes/Figures'
    figure(NF);
    Error(nE)=0;
    Eqk(iS).Stn(nE)
    if Eqk(iS).Stn(nE).Num > 154
        for iP=1:3
            ...
            Obse=bp_bu_co(t25'.*Syn0(iP).Signal,FMin,FMax,1/Dt,4,2);

            subplot(3,3,NS+3*(iP-1))
            plot(T,Obse,'b','linewidth',2.5), hold on
