% Modified: Brianna Birkel, Mar 2020
...
%% No modification needed in this section
...
% section according to your needs. Topography can be easyly included by 
...
m_plot(uclafaults(:,1),uclafaults(:,2),'color',[0 0 0],'linewidth',1)
...
while ~feof(FileID)
    ...
    if StN>1 && length(Chan)>2 && strcmp(Chan(1:2),'BH')
    ...
    elseif StN==1 && length(Chan)>2 && strcmp(Chan(1:2),'BH')
            Stn(StN).Ntwk = Ntwk;
            Stn(StN).Name = Name;
            Stn(StN).Lon = Lons;
            Stn(StN).Lat = Lats;
...
for StN=1:size(Stn,2)
    ...
    Stn(StN).X = CoorXY(2);
    Stn(StN).Y = CoorXY(1);
    if max(XLim)>CoorXY(2)+2e3 && CoorXY(2)-2e3>min(XLim)  ...
            && max(YLim)>CoorXY(1)+2e3 && CoorXY(1)>min(YLim)-2e3
            m_plot(Stn(StN).Lon,Stn(StN).Lat,'v',...
                'markerfacecolor',CA,'markeredgecolor',[0 0 0],...
                'MarkerSize',7,'color','k'); hold on
%             m_text(Stn(StN).Lon+0.1,Stn(StN).Lat,[Stn(StN).Name])
        StnUsf(iSt)=StN;
        iSt=iSt+1;
    end
    clear X Y CoorXY
...
for SrN=1:size(Eqk,2)
    ...
    CoorXY=[[cosd(Az) -sind(Az); sind(Az) cosd(Az)]*[Y-Y0_lon; X-X0_lat]]';
    Eqk(SrN).Loc.X = CoorXY(2);
    Eqk(SrN).Loc.Y = CoorXY(1);
...
save([PAuxFiles 'EqkStnDB.mat'],'Eqk','Stn');
