...
%    RSAC('endian','sacfile') reads in a SAC (seismic analysis code) binary
...
%
%    usage:  output = rsac('endian','sacfile')
%
%    Examples:
%
%    KATH = rsac('big-endian','KATH.R');
%    plot(KATH(:,1),KATH(:,2))
%
%    [SQRL, AAK] = rsac('big-endian','SQRL.R','AAK.R');
%
...
%fid  = fopen(sacfile,'r');
%[fileName, permission, endian]  = fopen(fid);
%fclose(fid);
