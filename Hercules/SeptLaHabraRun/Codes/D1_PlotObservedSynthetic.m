...
% NumEst - stations to plot based on index in Earthquake Database
...
for nE=NumEst
    ...
    SX = Factor*(SynS(nE).X);
    SY = Factor*(SynS(nE).Y);
    SZ = -Factor*(SynS(nE).Z); % because hercules saves vertical upside down 
    
    DN=Eqk(iS).Stn(nE).Cmpt(1).Signal;
    DE=Eqk(iS).Stn(nE).Cmpt(1).Signal;
    DZ=Eqk(iS).Stn(nE).Cmpt(1).Signal;
...
%         plot(T,Obse,'k','linewidth',2), hold on
        plot(T(1:size(Sint,2)),Sint,'r','linewidth',1.5),
