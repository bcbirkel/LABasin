...
for i = 1:length(nP)
    for j=  1:length(nF)
        G0 = sign(Phase(i,1).Freq(j,1).Kernels.Ks);
        Error1(i,j) = norm(Kf-G0);
    end
end

for i = 1:length(nP)
    for j=  1:length(nF)
        G1 = Phase(i,1).Freq(j,1).Kernels.Ks;
        Std1 = std(G1);
...
Error1 = Error1-min(min(Error1));
Error1 = Error1/max(max(Error1));

Error2 = Error2-min(min(Error2));
Error2 = Error2/max(max(Error2));
    
TotalE = 0.5*Error1+0.5*Error2;
Alpha = 1-TotalE;
...
subplot(2,2,1)
for j=  1:length(nF)
    plot(Error1(:,j),'-..','linewidth',1,'markersize',20), 
    hold on
end
axis 'square', xlim([0 length(nP)+1]), ylim([-0.1 1.1])
xlabel('Phase number'), ylabel('Misfit')
title 'a) Sensitivity', legend('f1','f2','f3')

subplot(2,2,2)
for j=  1:length(nF)
    plot(Error2(:,j),'-..','linewidth',1,'markersize',20), 
    hold on
end
axis 'square', xlim([0 length(nP)+1]), ylim([-0.1 1.1])
xlabel('Phase number'), ylabel('Misfit')
...
subplot(2,2,3)
for j=  1:length(nF)
    plot(TotalE(:,j),'-..','linewidth',1,'markersize',20), 
    hold on
end
axis 'square', xlim([0 length(nP)+1]), ylim([-0.1 1.1])
xlabel('Phase number'), ylabel('Misfit')
title 'c) Total misfit'

subplot(2,2,4)
for j=  1:length(nF)
    plot(Alpha(:,j),'-..','linewidth',1,'markersize',20), 
    hold on
end
axis 'square', xlim([0 length(nP)+1]), ylim([-0.1 1.1])
xlabel('Phase number'), ylabel('\alpha')
title 'd) Combination coefficient'


% Combine kernels
BASE = 0*Factor.BASE(i,:);
Ks = 0*Phase(1,1).Freq(1,1).Kernels.Ks;

for i = 1:length(nP)
    for j=  1:length(nF)
        if Alpha(i,j) > 0
            Ks = Ks+Alpha(i,j)*Factor.C(i,i,j)*Phase(i,1).Freq(j,1).Kernels.Ks;
            BASE = BASE+Alpha(i,j)*Factor.BASE(i,:);
        end
    end
end
...
figure;
subplot(2,2,1)
plot(t,sum(Factor.BASE,1),'k','linewidth',2), hold on

subplot(2,2,1)
plot(t,sum(BASE,1),'r','linewidth',1.5),
legend('Data','Localized','location','southeast')
xlim([0 t(end)]), ylim(max(abs(sum(Factor.BASE,1)))*1.5*[-1 1]),
title('Localization')
