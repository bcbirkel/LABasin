% Full Waveform Inversion - Code No F2.
% F1_KernelVizualization.m helps to plot profiles and maps for the
% kernels computed in F1_KernelComputation.m
% These kernel are the result from many or all the previous codes in the
% FWI sequence. In this code user only has to take
% care in sections where user modification is needed, like paths to
% databases and kernel outputs.
...
for iK = 1:9           % Number of kernel (depends on the database);
    nFq = 5;        % Frequency number (depends on the decomposition)
    NormD = 1e3;  % Distance for regularization of model parameters [m]
    
    %% No Modification Needed: Starting plots and calculations
    load([PathAdj 'KernelPhase' num2str(iK) 'Freq' num2str(nFq) '.mat']);
    ...
    % Computing Kernels for Mu and Kv, Eqs 18 and 19 in Tromp 2005
    % These are relative perturbation kernels
    Kmu=-2*Mu.*Kmu;
    Kka=-Ka.*Kka;
    
    % Computing Kernels for Vs and Vp
    Ks=2*(Kmu-(4/3)*(Mu./Ka).*Kka);
    Kp=2*((Ka+(4/3)*Mu)./Ka).*Kka;
    
    % % Model parameters regularization. Computing a Covariance Matrix
    %  Delete outliers with the standard Hampel method
    Ks(isnan(Ks))=0;
    Kp(isnan(Kp))=0;
...
%     
%     for i=1:length(Ks)
%         CovK=exp(-0.5*(((X-X(i)).^2+(Y-Y(i)).^2+2*(Z-Z(i)).^2).^0.5)/NormD);
%         KsR(i,1)=Ks'*CovK;
%         KpR(i,1)=Kp'*CovK;
%     end
%     
%     % Transform from relative perturbation to absolute perturbation
%     Ks=KsR./Vs;
%     Kp=KpR./Vp;
%     clear KsR KpR;
%     
    save([PathAdj 'KernelPhase' num2str(iK) 'Freq' num2str(nFq) '.mat'],'Ks','-append') ;
    save([PathAdj 'KernelPhase' num2str(iK) 'Freq' num2str(nFq) '.mat'],'Kp','-append') ;
...
%         'lat',[min(BoxSim(:,2)) max(BoxSim(:,2))]); hold on;
    ...
    CaxLi=3*std(Ks); caxis(CaxLi*[-1 1]); hold on; 
...
%     m_plot(BoxSim(:,1),BoxSim(:,2),'color',[0.5 0.5 0.5],'linewidth',2)
    ...
    set(gcf,'color','w')
    myaa([8 4])
    print(['./Figures/Phase' num2str(iK) 'Freq' num2str(nFq) '.png'],'-dpng')
    close all;
end
