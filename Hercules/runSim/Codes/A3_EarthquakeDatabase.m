...
%% NOTE FROM BRIANNA: example has one folder ObsData/Velocity/ with velocity
%% files from IRIS (SAC) + SACPZ files all together. Must weed out any data
%% files with less than three components for later codes to work. 
%% These are called below; this section doesn't seem to do anything but set the path.
...
%             ArS=char(AVel{iAv});
            ...
            pz_file = [PathRes 'Velocity/SACPZ/' 'SACPZ.' ArS(1:IP(2))  ...
                StnPos '.' ChaHead ArS(IP(3)+3:IP(4)-1)];
    ...
    % %             Signal = Signal-conv(sgolayfilt(Signal,2,a),(1/a)*ones([1 a]),'same');
                ...
                sacfile = [PathWF 'Velocity/' char(AVel{iAv})];
                movefile(sacfile, [PathWF 'Velocity/noPZfile/'])
            end
        end
    end
...
for iS=1:size(Eqk,1)
    if isempty(Eqk(iS).Stn) || size(Eqk(iS).Stn,2)==0 
        ...
        CleanS=[]; nS=1;
        for iA=1:size(Eqk(iS).Stn,2)
            if length(Eqk(iS).Stn(iA).Cmpt)==3
...
%             if length(Eqk(iS).Stn(iA).Cmpt)==2
%                 if norm(Eqk(iS).Stn(iA).Cmpt(1).Signal)==0 ...
%                         && norm(Eqk(iS).Stn(iA).Cmpt(2).Signal)==0
%                     CleanS(nS)=iA;
%                     nS=nS+1;
%                 end
%             end
%             if length(Eqk(iS).Stn(iA).Cmpt)==1
%                 if norm(Eqk(iS).Stn(iA).Cmpt(1).Signal)==0
%                     CleanS(nS)=iA;
%                     nS=nS+1;
%                 end
