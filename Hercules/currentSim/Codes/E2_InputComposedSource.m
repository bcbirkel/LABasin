...
Finp = fileread([PathAux 'inputDUDE.in']);
...
subplot(3,2,1)
plot(t,sum(BASE(1:MMax,:),1)','r','linewidth',2),
legend('Data D(t)','\SigmaAtoms','location','southeast')
xlim([0 t(end)]), ylim(max(abs(SynF))*[-1 1]),
grid minor, title('Reconstruction')
xlabel('Time [s]') , ylabel('Displacement [s]') 

subplot(3,2,[2 4])
for m = 1:MMax 
    plot(t,m+BASE(m,:)/max(max(abs(BASE))),'k','linewidth',2), hold on
end
xlim([0 t(end)]), ylim([0 m+1]), grid minor,
xlabel 'Time [s]', ylabel 'Atoms'
title('Dictionary'), 

SynF=sum(BASE(1:MMax,:),1)';
...
%An extra using the entire seismogram
cd (PathSim)
NameDir = 'Phase';
DirSource= strcat(NameDir,num2str(88));
mkdir(DirSource);
cd(DirSource);
...
fprintf(fileID,'%i\t',length(FWC.ASF));
fprintf(fileID,'%i\t',0);
fprintf(fileID,'%6.4f\t',dt);
fprintf(fileID,'%e\t',FWC.ASF);
fprintf(fileID,'\n');
fclose(fileID);
clear fileID;
