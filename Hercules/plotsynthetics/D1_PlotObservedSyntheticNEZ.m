...
load('../hercules_preprocessing/MatlabCodes/DataPreparation/EqkStnDB.mat');
...
FileID = fopen('..                                                                                                                 /hercules_preprocessing/MatlabCodes/AuxFiles/XYtoLonLat.txt');
...
for nr = 1:PointsStrike
    ...
    %Rotate Syn XY to NE (commented out XY to RT)
    %Syn(1).Signal = cosd(AnR)*SY+sind(AnR)*SX;
    %Syn(2).Signal = -sind(AnR)*SY+cosd(AnR)*SX;
    Syn(1).Signal = -sind(An)*SX+cos(An)*SY
    Syn(2).Signal = cosd(An)*SX+sin(An)*SY
    Syn(3).Signal = SZ;
    
    clear DX DY DZ SX SY SZ DN DE;
    ...
    for iP=1:3
        ...
        xlabel 'Time [s]', legend('Observed','Synthetic'), 
        xlim([0 T(end)]), ylim(Am*[-1 1]),
        ylabel('Vel [m/s]'),
        text(2.5,0.25*Am,[Eqk(ns).Stn(nr).Name  CMP(iP,:)])
    end
    saveas(gcf, ['SynObs_' Eqk(ns).Stn(nr).Name '.png'])
    NS=NS+1;
    if NS>3
        NS=1; NF=NF+1;
