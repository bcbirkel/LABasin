...
if ~isfield(para.vario,'model')
  message(1,'Need to load parameter file first !!');
  return
end
...
if min(data.in.var1) > xmax_set | max(data.in.var1) < xmin_set
   message(1,['X-coordinate settings are inconsistent with the input data, click on ''Refresh'' button to ' ...
...
if min(data.in.var2) > ymax_set | max(data.in.var2) < ymin_set
   message(1,['Y-coordinate settings are inconsistent with the input data, click on ''Refresh'' button to ' ...
   'get automatic settings !!']);
    return
end
if data.in.dim == 3
   ...
   zmax_set=str2num(get(hdl.krig.zmax,'string'));
	if min(data.in.var3) > zmax_set | max(data.in.var3) < zmin_set
   	message(1,['Z-coordinate settings are inconsistent with the input data, click on ''Refresh'' button to ' ...
   	'get automatic settings !!']);
    return
	end
end
...
if para.krig.batch_file_proc == 1
 %%% 1/5/02   
  if para.krig.bat_proc_cnt == 1
     ...
     hdl.krig.hmsg=message(1,msg1);
  else
     msg1=[sprintf('Kriging on file # %d is done in %g (sec), \n\nTotal time = %g (sec)',para.krig.bat_proc_cnt,round(t1),para.krig.bat_proc_time)];
     message(3,msg1,' ',hdl.krig.hmsg);
