...
while(1)
    ...
    A(1:3:end,6)=-m*((-cosey*sinez)*(datum1(:,1)-rc(1))+(-sinex*siney*sinez+cosex*cosez)*(datum1(:,2)-rc(2))+(+cosex*siney*sinez+sinex*cosex)*(datum1(:,3)-rc(3)));
    ...
    r=size(A,1)-size(A,2);
    %Pbb=inv(Qbb);
    Pbb = speye(3*s1(1));   % Accuracy of points is not used yet
    APbbA = A'*Pbb*A;
