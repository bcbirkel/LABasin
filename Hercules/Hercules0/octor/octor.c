/***
 *
 * octor.c: Parallel unstructured octree mesh generation library.
 *
...
typedef struct mem_t {
    ...
    int32_t objsize;        /* size of each individual data object */
    int32_t chunkcapacity; 
    int32_t chunkdatasize;  /* size of the real data, excl. hdr_t and tlr_t */
    int32_t chunkbytesize;  /* size of a chunk (data + hdr_t + tlr_t */
    int32_t chunkcount;     /* Number of chunks allocated */

    void *recycled;         /* pointer to the recycled objects */
    void *first;            /* pointer to the first allocated memory chunk */
    void *last;             /* pointer to the last allocated memory chunk */
...
static mem_t * mem_new(int32_t objsize, int32_t chunkcapacity);
...
static void    mess_showstat(mess_t *mess, int32_t mode, const char *comment);
...
/**
 * stack_t: This stack should never overflow in this program.
 *
 */
...
static oct_t * oct_findneighbor(oct_t *oct, dir_t dir, stack_t *stackp);
...
static int8_t  tree_getminleaflevel(tree_t *tree);
static int8_t  tree_getmaxleaflevel(tree_t *tree);
...
static oct_t * tree_ascend(oct_t *oct, dir_t I, stack_t *stackp);
static oct_t * tree_descend(oct_t *oct, stack_t *stackp);
static int32_t tree_pushdown(oct_t *nbr, tree_t *tree, stack_t *stackp, 
                             setrec_t *setrec);
...
/**
 * gnid_info_t: Global node identifier message.
 *
 */
typedef struct gnid_info_t {
    tick_t x, y, z;
    int64_t gnid;
} gnid_info_t;
...
/**
 * pctl_t: (Remote) processor controller.
 *
 */
...
static void      com_OrchestrateExchange(com_t *com, int32_t msgtag);
 ...
 */
static mem_t *
mem_new(int32_t objsize, int32_t chunkcapacity)
{
    mem_t *mem;
...
mem_newobj(mem_t *mem)
{
    ...
    if (mem->recycled != NULL) {
        ...
        if ((hdr == NULL) || (hdr->used == hdr->capacity)) {
            ...
            newtlr->nextobj = (char *)newchunk + sizeof(hdr_t);
        ...
        tlr->nextobj = (char *)tlr->nextobj + mem->objsize;
        hdr->used++;
    }
 ...
 */
static oct_t *
oct_findneighbor(oct_t *oct, dir_t dir, stack_t *stackp)
{
...
oct_sprout(oct_t *leafoct, tree_t *tree)
{
    ...
    /* Ajust the firstleaf pointer to point to the first child */
...
extern int32_t
octor_getminleaflevel(octree_t *octree, int where)
{
    tree_t *tree = (tree_t *)octree;
    int32_t lmin, gmin;
    ...
    if (where == LOCAL) {
        ...
        if (tree->groupsize > 1) {
            MPI_Allreduce(&lmin, &gmin, 1, MPI_INT, MPI_MIN, MPI_COMM_WORLD);
            return gmin;
        } else {
            return lmin;
        }
...
extern int32_t
octor_getmaxleaflevel(octree_t *octree, int where)
{
    tree_t *tree = (tree_t *)octree;
    int32_t lmax, gmax;
 ...
 */
static int32_t
tree_setcom(tree_t *tree, int32_t msgsize)
{
 ...
 */
static oct_t *
tree_ascend(oct_t *oct, dir_t I, stack_t *stackp)
{
    dir_t O, opposite;
 ...
 */
static oct_t *
tree_descend(oct_t *oct, stack_t *stackp)
{
    while (stackp->top > 0) {
 ...
 */
static int32_t
tree_pushdown(oct_t *nbr, tree_t *tree, stack_t *stackp, setrec_t *setrec)
{
    dir_t dir;
        ...
        for (k = 0; k < 4; k++) {
            ...
            for (j = 0; j < 4; j++) {
                ...
                for (i = 0; i < 4; i++) {
                    ...
                    /* Ignore all the half-octs on the diaganals */
                    /* 
                       if (((i == j) && (j == k)) ||
                       ((3 - i == j) && (3 - i == k)) ||
                       ((3 - j == i) && (3 - j == k)) ||
...
/**
 * com_resetpctl: Reset each existing pctl. However, the communicaiton 
 ...
 */
static void 
com_OrchestrateExchange(com_t *com, int32_t msgtag)
{
                ...
                if (pctl->sndactive != &pctl->sndmem) {
                    ...
                    if (pctl->sndactive == NULL) {
                        ...
                        MPI_Isend(pctl->sndactive, sendbytes, MPI_CHAR, 
                                  pctl->procid, msgtag, MPI_COMM_WORLD,
                                  &isendreqs[isendnum]);
                        isendnum++;
 ...
 */
static int32_t 
node_setproperty(tree_t *tree, vertex_t *vertex, unsigned char *pproperty)
{
    tick_t nx, ny, nz;
    int32_t where, onX, onY, onZ;
    int32_t modX, modY, modZ;
    tick_t masterMask;
    int8_t masterLevel;

    if (vertex->touches == 8) {
        /* This must be an anchored node */

        *pproperty = 0X80;
        return 0;
    }

    /* Reverse possible coordiante adjustment */
    nx = vertex->x;
    ny = vertex->y;
    nz = vertex->z; 

    onX = ((nx == tree->nearendp[0]) || (nx == tree->farendp[0] )) ? 1 : 0;
    onY = ((ny == tree->nearendp[1]) || (ny == tree->farendp[1] )) ? 1 : 0;
    onZ = ((nz == tree->nearendp[2]) || (nz == tree->farendp[2] )) ? 1 : 0;

    where = onX + onY + onZ;

    switch (vertex->touches) {
    case (6) : 
        /* This must be an internal dangling node */
        if (where != 0) 
            return -1;
    ...
    case (4):
        ...
        if (where == 1) {
            ...
            *pproperty = 0X80;
            ...
            /* Internal dangling node, might be face or edge */
    ...
    case (2) :
        ...
        if (where == 2) {
            /* On boundary edge, must be an anchored node */
            *pproperty = 0x80;

        } else {
            /* Internal or boundary edge-dangling node */
...
octor_newtree(double x, double y, double z, int32_t recsize,
              int32_t myid, int32_t groupsize)
{
    tree_t *tree;
    ...
    */
    ...
    for (level = gmax; level > threshold; level--) {
            ...
            for (dir = L; dir <= UF; dir = (dir_t) ((int)dir + 1)) {
                ...
                if (nbr->where == LOCAL) {
                    ...
                    if ((nbrLDB.x < tree->nearendp[0]) ||
                        ...
                        /* Ingore the out-of-bound neighbor oct */
                        continue;
                    }
        ...
        if (tree->groupsize > 1) {
            ...
            com_OrchestrateExchange(tree->com, DESCENT_MSG);
 ...
 */
extern int32_t 
octor_partitiontree(octree_t *octree)
{
    tree_t *tree = (tree_t *)octree;
    ...
    while (due > 0) {
        ...
        if (bin_procid != tree->procid) {
            ...
            MPI_Irecv(rcvleafoct_pool, rcvbytesize, MPI_CHAR, bin_procid,
                      OCT_MSG, MPI_COMM_WORLD, &irecvreqs[irecvnum]);
 ...
 */
extern mesh_t *
octor_extractmesh(octree_t *octree)
{
    tree_t *tree = (tree_t *)octree;
    ...
    for (eindex = 0; eindex < ecount; eindex++) {
        ...
        if (oct == NULL) {
            fprintf(stderr, "Thread %d: %s %d: internal error (too few octs)\n"
                    , tree->procid, __FILE__, __LINE__);
            MPI_Abort(MPI_COMM_WORLD, INTERNAL_ERR);
            exit(1);
        }
    ...
    if (tree->groupsize > 1) {
                    ...
                    for (ytick = 0; ytick < 2; ytick++) {
                        ...
                        for (xtick = 0; xtick < 2; xtick++) {
                            ...
                            if (procid == tree->procid) {
                                ...
                                /* Assume it's the first occurence */
                                existed = 0;
        ...
        com_OrchestrateExchange(tree->com, VERTEX_INFO_MSG);
    ...
    for (hashentry = 0; hashentry < ecount; hashentry++) {
        ...
        while (link != NULL) {
            ...
            if (node_setproperty(tree, vertex, &vertex->property) != 0) {
                fprintf(stderr, "Thread %d: %s %d: ", 
                        tree->procid, __FILE__, __LINE__);
                fprintf(stderr, "node_setproperty() error\n");
                MPI_Abort(MPI_COMM_WORLD, INTERNAL_ERR);
                exit(1);
            }
    ...
    if (tree->groupsize > 1) {
        ...
        /* Exchange the ANCHORED_MSG to finish builing the sharing
           relationship */
        com_OrchestrateExchange(allcom, ANCHORED_MSG);
    ...
    if (tree->groupsize > 1) {
        ...
        com_OrchestrateExchange(partcom, GNID_MSG);
