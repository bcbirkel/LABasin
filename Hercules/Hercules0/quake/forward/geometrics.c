...
vector3D_t compute_global_coords( vector3D_t origin, vector3D_t local,
  ...
  xGlobal.x[ 0 ] =( cos( p ) * cos( l ) + sin( p ) * cos( d ) * sin( l )) * x -
    (-cos( p ) * sin( l ) + sin( p ) * cos( d ) * cos( l )) * y -
    (-sin( p ) * sin( d )                     ) * z;  
  xGlobal.x[ 1 ] =( sin( p ) * cos( l ) - cos( p ) * cos( d ) * sin( l )) * x -
    (-sin( p ) * sin( l ) - cos( p ) * cos( d ) * cos( l )) * y -
    ( cos( p ) * sin( d )                     ) * z;
  xGlobal.x[2]= -sin( d ) * sin( l ) * x +
    sin( d ) * cos( l ) * y +
    cos( d ) *z;  
...
/* 
 *  compute_domain_coords: rotates the x domain and y domain components 
 *                         if the rectangular prism is not alinged with
 *                         the north.
 *                  input:
 *                         azimuth in degrees;
 *   
...
vector3D_t compute_domain_coords(vector3D_t point, double azimuth){
  ...
  for ( iRow = 0; iRow < 3; iRow++)
    for ( iCol = 0; iCol < 3; iCol++)
      pointTransformed.x[iRow] += transformation[iRow][iCol] * point.x[iCol];
...
/* 
 *  compute_domain_coords_linearinterp: computes the domain coordinates
...
/* Compute bilinear transformation from XYZ to LonLatz                     */
/*                                                                         */
/* the orientations is clockwise and there and the initial node is the     */
/* lower left corner                                                       */

int compute_lonlat_from_domain_coords_linearinterp(double y , double x , 
						   double *lon,double *lat,
						   double *longcorner,   double *latcorner,
						   double domainlengthX, double domainlengthY){
    int i;
    double lonI[4],latI[4], X, Y, phi;  
    double Xi [4] = {-1, +1,  +1, -1};
    double Yi [4] = {-1, -1,  +1, +1}; 
    ...
    X=(x-domainlengthX/2)/(.5*domainlengthX);
    Y=(y-domainlengthY/2)/(.5*domainlengthY);
