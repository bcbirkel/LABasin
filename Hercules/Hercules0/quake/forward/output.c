 ...
 *
 * File info:   $RCSfile: output.c,v $
 *              $Revision: 1.19 $
 *              $Date: 2007/07/13 00:39:33 $
 *              $Author: jclopez $
 */
...
po_open_4d_file (const char* filename, out_hdr_t* out_hdr, int my_rank)
{
    ...
    MPI_Bcast (&tmp, 1, MPI_CHAR, 0, MPI_COMM_WORLD);
...
po_fini (par_output_t* po)
{
     ...
     * check the file size.
     */
    MPI_Reduce (&sendval, &recvval, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
	...
	)
{
    ...
    MPI_Gather (&count, 1, MPI_INT, count_table, 1, MPI_INT, 0, MPI_COMM_WORLD);
	...
	)
{
    ...
    MPI_Reduce (&st->tput_min, &min,  1, MPI_FLOAT, MPI_MIN, 0, MPI_COMM_WORLD);
    MPI_Reduce (&st->tput_max, &max,  1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce (&st->tput_avg, &mina, 1, MPI_FLOAT, MPI_MIN, 0, MPI_COMM_WORLD);
    MPI_Reduce (&st->tput_avg, &maxa, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce (&st->tput_avg, &sum,  1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce (&osum2,	       &sum2, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce (&st->tput_var, &minv, 1, MPI_FLOAT, MPI_MIN, 0, MPI_COMM_WORLD);
    MPI_Reduce (&st->tput_var, &maxv, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce (&st->tput_var, &avgv, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce (&st->iops, &avg_iops, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
	...
	)
{
	...
	if (0 == po->pe_id && NULL != fp) {
	    ...
	    ret = MPI_Attr_get (MPI_COMM_WORLD, MPI_WTIME_IS_GLOBAL,
