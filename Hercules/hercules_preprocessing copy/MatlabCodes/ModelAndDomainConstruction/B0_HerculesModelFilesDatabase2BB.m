% Process CVM426
% Written by Alan Juarez (UNAM-USC) Dec, 2016. Edited for Earthquake
% Physics in Nov,2019
clear; close all; clc;

% Load XYZ mesh data
Mesh = load(['/home/scec-00/birkel/hercules_preprocessing/MatlabCodes/AuxFiles/VelModelCoords.xyz']);
X = Mesh(:,1);
Y = Mesh(:,2);
clear Mesh;

% Load UCVM model
fileID = fopen('/home/scec-00/birkel/hercules_preprocessing/MatlabCodes/AuxFiles/ModelOut.out','r');
...
for ix=1:lx
    for iy=1:ly
        for iz = 1:lz
            ...
            if Z(nP) < 600 && Vs(nP) < 1005
                Beta(ix,iy,iz) = (0.3*Z(nP)/500+0.7)*Vs(nP);
            else
                Beta(ix,iy,iz) = Vs(nP);
            end
...
% depths
FileW = fopen('vel_profiles_unidad1_DB2.fun','w');
...
P = 0:lx*ly-1;
fileX = fopen('vel_profiles_unidad1_DB2.surf','w');
