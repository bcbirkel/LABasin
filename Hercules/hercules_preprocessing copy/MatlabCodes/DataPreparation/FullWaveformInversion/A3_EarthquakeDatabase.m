...
PathAux = '/home/dude/alanjuar/CoachellaValley/Codes/AuxFiles/';
...
PathRes='/home/dude/alanjuar/CoachellaValley/AllData/';
...
PathWF='/home/dude/alanjuar/CoachellaValley/Resources/AllData/';
...
for iS=1:size(Eqk,1)
    ...
    for iAv=1:size(Vel,1)
        if isempty(Vel{iAv})==0
            ...
            SF=rsac('little-endian',[PathWF  char(AVel{iAv})]);
...
%                     if (2*Ener1 < Ener2) %&& norm(Signal) < 1
%                         figure(iA)
%                         subplot(3,1,iC)
%                         Tiempo=0:Dt:Dt*(length(Signal)-1);
%                         plot(Tiempo,Signal), hold on
