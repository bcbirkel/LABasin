...
function [maxdisp, maxvel, maxacc] = max_osc_response_fast( acceleration, dt, csi, ...
    ...
    DV=[[ca  cb  cc  cd ];[cap cbp ccp cdp ]];
    dvmo=zeros(np,2);
    for  i=2:np
        mo=[dvmo(i-1,1) dvmo(i-1,2) acceleration(i-1) acceleration(i)];
        dvmo(i,:)=(DV*mo')';
%        d(i)=dvmo(i,1);
%        v(i)=dvmo(i,2);
    end
    
    aa=-ww*dvmo(:,1)-dcsiw*dvmo(:,2);
    maxdisp =max(abs(dvmo(:,1)));
    maxvel  =max(abs(dvmo(:,2)));
    maxacc  =max(abs(aa));           
