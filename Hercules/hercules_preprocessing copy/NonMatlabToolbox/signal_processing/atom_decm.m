function [S_BASE,C_TF] = atom_decm(ds,dt,FMin,FMax,K)
...
Signif = spec_signif(ds,0.005);  % Spectral significancy at 99% (works good for synthetic data)
...
% Start plot
% Isolate phases with the watershed algorithm
L = double(watershed(-log2(Ws)));   % Change number format
[X1,X2] = meshgrid(T,F);

r = 1;      % Phase counter (not all the values will be keept)
for i = 1:length(loct)
    iFilt = 0*L;        % Initiate isolation filter = phase
    iFilt(L == L(locf(i),loct(i))) = 1;
    iFilt = imgaussfilt(iFilt,4);

    TF = [T(loct(i)) F(locf(i))];
    Sigma = [1/F(locf(i))/4 0; 0 4*F(locf(i))];
    G = mvnpdf([X1(:) X2(:)],TF,Sigma);
    G = reshape(G/max(G),length(F),length(T));
    
    FiltroI = istransform(iFilt.*G.*sts)'; % Estimate inverse transform of isolated phase
...
% xlabel('Time (s)'), ylabel('Frequency (Hz)'), caxis([0 0.5*std(Ws(:))])
% CM = loadcmap('WhBlGrYeRe.c3g'); colormap(CM),
% 
% % subplot(2,2,3)
% % surf(T,F,log2(Ws))
% % shading interp, view (2), set(gca,'yscale','log'), hold on
