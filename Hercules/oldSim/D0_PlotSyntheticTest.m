...
% saved the outputs from the Hercules Simulation 20/2 20/3 24/2 24/3 19/3 23/2 
PathSrc = './planes/';
...
PathPlane=[PathSrc 'planedisplacements.0'];
...
for nE=28:31%PointsStrike
    Syn(1).Signal = SynS(nE).X;
    Syn(2).Signal = SynS(nE).Y;
    Syn(3).Signal = SynS(nE).Z; 
