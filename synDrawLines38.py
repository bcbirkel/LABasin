...
for kk in iis:
    for j in ms:
            ...
            if rotateall == True:
                ...
                for i in range(len(stN)):
                    if stN[i].stats.xcoord == stE[i].stats.xcoord and stN[i].stats.ycoord == stE[i].stats.ycoord:
                        ...
                        # trR.filter("bandpass", freqmin=1/10, freqmax=1/5)
                        # trT.filter("bandpass", freqmin=1/10, freqmax=1/5)
                        stRall.append(trR)
                        stTall.append(trT)
                ...
                plt.xlim(0,60)
                plt.ylim(0,80)
                plt.title("Transverse - " + event + ", " + model)
                plt.xlabel("Time (seconds)")
                plt.ylabel("Distance from source (km)")
                plt.show()
            
                print("check ROTATED plots")
        ...
        for ii in range(len(lat1)):
            ...
            def intermediates(p1, p2, nb_points=int(dist/500)):
            ...
            for lnpt in linepts: # points along line
                ...
                ind = 0
                for pt in xymod_sorted:    
                    len_arr[ind] = math.dist([pt[2], pt[3]],[lnpt[0], lnpt[1]])
                    ind += 1
                    ...
                    if stN[i].stats.xcoord == stE[i].stats.xcoord and stN[i].stats.ycoord == stE[i].stats.ycoord:
                        for pt in savedlnpts:
                                if stN[i].stats.xcoord == pt[0][0] and stN[i].stats.ycoord == pt[0][1]:
                                    ...
                                    trT.stats.cmp = 'T'
                                    trR.data, trT.data = rotate_ne_rt(stN[i].data,stE[i].data,stN[i].stats.baz)
                                    # trR.filter("bandpass", freqmin=1/10, freqmax=1/2)
                                    # trT.filter("bandpass", freqmin=1/10, freqmax=1/2)
                                    stR.append(trR)
                                    stT.append(trT)
            ...
            for tr in stR:
                linedist = gps2dist_azimuth(lat1[ii], lon1[ii], tr.stats.lat, tr.stats.lon)
                a.plot(tr.times(),tr.data*10+linedist[0]/1000,color='k',lw=0.5)
            ...
            #     [sourcedist1,sourceaz1,sourcebaz1] = gps2dist_azimuth(eventlat, eventlon, lat1[ii], lon1[ii])
            #     [sourcedist2,sourceaz2,sourcebaz2] = gps2dist_azimuth(eventlat, eventlon, lat2[ii], lon2[ii])
                
            #     if sourcebaz1 > sourcebaz2:
            #         bazHigh = sourcebaz1
            #         bazLow = sourcebaz2
            ...
            #     for i in range(len(stN)):
            #         if stN[i].stats.xcoord == stE[i].stats.xcoord and stN[i].stats.ycoord == stE[i].stats.ycoord:
            #             [linedist,az,baz] = gps2dist_azimuth(lat1[ii], lon1[ii], stN[i].stats.lat, stN[i].stats.lon)
                        
            #             if stN[i].stats.baz < bazHigh and stN[i].stats.baz > bazLow: 
            #                 print(stN[i].stats.baz)
            #                 trR = stN[i]
            #                 trT = stE[i]
            #                 trR.stats.cmp = 'R'
            #                 trT.stats.cmp = 'T'
            ...
            #     # for tr in stRaz:
            #     #     xx,yy = m(tr.stats.lon,tr.stats.sac.lat)
            #     #     m.scatter(xx,yy, marker = "." ,s=2, color='b', alpha = 1)
            ...
            #     for tr in stRaz:
            #         if tr.stats.dist < 50:
            #             a.plot(tr.times(),tr.data*2+tr.stats.dist,color='k',lw=0.2)
                            

            #     plt.xlim(0,60)
            #     plt.ylim(0,50)
            #     plt.title("Radial - baz from source along " + name [ii] + " - " + event + ", " + model)
            #     plt.xlabel("Time (seconds)")
            #     plt.ylabel("Distance from source (km)")
            #     plt.show()
            ...
            #     plt.xlim(0,60)
            #     plt.ylim(0,50)
            #     plt.title("Transverse - baz from source along " + name [ii] + " - " + event + ", " + model)
            #     plt.xlabel("Time (seconds)")
            #     plt.ylabel("Distance from source (km)")
            #     plt.show()
            
            #     print("check azimuth plots")
