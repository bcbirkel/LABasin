...
for event_no in ks:
    for j in ms:
        ...
        dataPath = './CompiledEvents/' + event + '/JS_Syn/'
        ...
        if event_no == 0:
            ...
            # start = UTCDateTime("2014-03-29T04:09:42")  
        ...
        # for sytr in st_syn:
        #     if sytr.stats.channel == "BHN":
        #         sytr.stats.channel = "E"
        #     if sytr.stats.channel == "BHE":
        #         sytr.stats.channel = "N"
                
        # for sytr in st_syn:
        #    if sytr.stats.channel == "E":
        #        sytr.stats.channel = "BHE"
        #    if sytr.stats.channel == "N":
        #        sytr.stats.channel = "BHN" 
                # print(tr.stats)
        ...
        if plotTraces == True:
            ...
            st_data.filter('bandpass',freqmin=lowfreq,freqmax=highfreq,zerophase=False)
            st_syn.filter('bandpass',freqmin=lowfreq,freqmax=highfreq,zerophase=False)
            
            # stalist = ["OLI","WLT","SRN","DLA","CHN"]
            ...
            for i in range(3):
                ...
                a.set_xlim(0,xmax)
                a.set_ylim(0,ymax)
                a.set_title(name + " " + mod + " " + cha + " - Buka syn (black) vs RG grid synthetics (blue) - bandpass: 5-10 seconds")
                count=0
                datastns=[]
                for tr in st_data:
                    if tr.stats.channel == cha and tr.stats.station in stalist and tr.stats.sac.dist < ymax:
                        ...
                        #     print(str(tr))
                        if str(tr)[0:2] == 'CE' or str(tr)[0:2] == 'CI' :
                        #     tr.filter("lowpass", freq=lowfreq)
                            # a.plot(tr.times(reftime=start), tr.data*gain+tr.stats.sac.dist, c="y",linewidth=0.5)
                            # a.text(xmax-5,tr.stats.sac.dist+0.2,tr.stats.station)
                            count = count + 1
                            if count%3==0:
                                a.plot(tr.times(reftime=start)+6.6, tr.data*gain+tr.stats.sac.dist, c="k",linewidth=0.5)
                                a.text(xmax-5,tr.stats.sac.dist+0.2,tr.stats.station + " max amp: " + str(max(abs(tr.data))))
                                datastns.append(tr.stats.station)
                        # count = count + 1
                count = 0        
                for sytr in st_syn:
                    if sytr.stats.channel == cha and sytr.stats.station in datastns and sytr.stats.sac.dist < ymax:
                        # if str(tr)[0:2] == 'CI':
                        #     tr.filter("lowpass", freq=lowfreq)
                            
                            
                        # if sytr.stats.station!="RHC2" and sytr.stats.station!="RIO" and sytr.stats.station!="BRE" :
                            if cha!='BHZ':
                                # count = count + 1
                                # if count%5==0:
                            ...
                            if cha=='BHZ':
                                ...
                                # if count%5==0:
                                    a.plot(sytr.times(reftime=start), -sytr.data*gain+sytr.stats.sac.dist, c="b",linewidth=0.5)
                            a.text(xmax-5,sytr.stats.sac.dist-1,sytr.stats.station + " max amp: " + str(max(abs(sytr.data))),c='b')
                            # print(str(tr))
                            # count = count +1
