...
def plot_syn_JS(eventfile, event_title, vmodel, stations, sta_names, event_lat, event_lon):
...
#                    comp_Z = [scale_fact*x for x in comp_Z] 
    ...
    # %% fill header            
                    ...
                    stats = Stats()
                    header={'times': time, 'delta': (float(time[1])-float(time[0])), 'npts': len(time), 'sampling_rate': 1/(float(time[1])-float(time[0])),'kstnm': sta_name, \
                            'stla': sta_lat, 'stlo': sta_lon, 'evla': event_lat, 'evlo': event_lon}
                    #stats.network= 'CI'
                    #stats.channel = 'ZNE'
                    stats.npts = len(time)
...
#                    stN.filter('bandpass', freqmin=freqmin, freqmax=freqmax, corners=4, zerophase=False)
        ...
        # %% convert to SAC and save files
                    os.chdir('/Users/bcbirkel/Documents/GitHub/LABasin/CompiledEvents/' + event + "/GravesSyn/CVM-" + mod_title + "/JSsyn/")
                    stN.write("Syn_" + event_title + "_" + sta_name + "_N.SAC")
                    stE.write("Syn_" + event_title + "_" + sta_name + "_E.SAC")
                    stZ.write("Syn_" + event_title + "_" + sta_name + "_Z.SAC")
